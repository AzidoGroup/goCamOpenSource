      ...
      export namespace Ml {
        ...
        export class Tesseract extends Avs.Plugin.Handler {
          ...
          public extractInformationFromId(image: any, cb: ((event: any) => void)) {
            ...
            this.ocrImage(image, (result: any) => {
              ...
              if (findDate.length >= 4) {
                ...
                const dateComponentsPosition = this.config.idConfig.dateComponentsPosition[ruleMatched];
                const dayIndex               = dateComponentsPosition.day;
                const monthIndex             = dateComponentsPosition.month;
                const yearIndex              = dateComponentsPosition.year;

                const day   = this.formatDay(findDate[dayIndex]);
                const month = this.formatMonth(findDate[monthIndex]);
                const year  = this.formatYear(findDate[yearIndex]);

                const dateFoundString = year + '-' + month + '-' + day;
                const dateFound       = Date.parse(dateFoundString);

                const dateNow = (+new Date());

                // 365.2425 = days in a year with leap day added
                const scanIdPersonYears = Math.floor(((dateNow - dateFound) / 1000 / 3600 / 24 / 365.2425));
          ...
          public formatMonth(month: string): string {
            ...
            for (const i in this.monthNameList[this.monthNameLanguage]) {
