    ...
    export class Listener {
      ...
      private removeContextFromList(context: any, eventList: { [index: string]: Array<any>; }) {
        for (const event in eventList) {
          let indice = eventList[event].length;
          while (indice--) {
            const eventData = eventList[event][indice];
      ...
      public fire(eventName: string, params?: any): any {
        ...
        const isInNormal = eventName in this.eventList;
        const isInBefore = eventName in this.eventListBefore;
        const isInAfter  = eventName in this.eventListAfter;
      ...
      private forward(observateurList: Array<any>, argList: any) {
        let i: number = observateurList.length;
        let r: boolean;
        while (i--) {
          let delay: number = 0;
          const fnctInfo      = observateurList[i];
      ...
      public registerEventName(eventName: string, fnct: any, context: any, options?: IListenerOption, position?: number) {
        ...
        const eventData = [fnct, context, options];
