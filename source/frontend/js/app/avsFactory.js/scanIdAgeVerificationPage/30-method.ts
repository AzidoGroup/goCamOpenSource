  ...
  export namespace ScanIdAgeVerificationPage {
    ...
    export class Method {
      ...
      public static extractFace() {
        ...
        const processingCanvas = Method.getProcessingCanvas();
        ...
        instance.plugin.Library.Ml.FaceApi.detectFaceFromCustomElement(processingCanvas, (faceResult: any) => {
          ...
          if (faceResult !== null) {
            ...
            const adjustment = Method.getBrightnessAndContrastRandomAdjustment();
      ...
      public static getBrightnessAndContrastRandomAdjustment() {
        ...
        const negativeBrightnessOperation = Math.random() >= 0.5;
        const negativeContrastOperation   = Math.random() >= 0.5;
        const brightnessAdjustment        = (Math.floor(Math.random() * Config.BRIGHTNESS_ADJUSTMENT_MAX_VALUE) + 1);
        const contrastAdjustment          = (Math.floor(Math.random() * Config.CONTRAST_ADJUSTMENT_MAX_VALUE) + 1);
      ...
      public static compareFaceDescriptors() {
        ...
        for (let i = 0, j = instance.entity.SelfieAgeDetection.validCanvasFaceList.length; i < j; i++) {
          for (let ii = 0, jj = instance.entity.ScanIdAgeVerification.validFaceList.length; ii < jj; ii++) {
            ...
            const distance = instance.plugin.Library.Ml.FaceApi.compareFaceDescriptors(
            ...
            const faceItsMatching = distance <= Config.MAX_FACE_DISTANCE_TO_MATCH;
            if (faceItsMatching) {
      ...
      public static ocrImage() {
        ...
        instance.plugin.Library.Ml.Tesseract.setIdConfig(instance.entity.ScanIdAgeVerification.getIdTypeConfig(), (workerWasConfigured: boolean | null) => {
          ...
          const idSizeConfig          = instance.entity.ScanIdAgeVerification.getIdTypeConfig().idSizeList;
          const faceCoordinates       = instance.plugin.Library.Ml.FaceApi.extractFaceCoordinates(instance.entity.ScanIdAgeVerification.validFaceList[0]);
          const birthDateMaskPosition = Avs.Helper.ElementPositionCalculator.calculateBirthDateMaskPosition(faceCoordinates, idSizeConfig);
          const canvasResultElement   = $(Config.CAMERA_SOURCE_CANVAS_RESULT_ELEMENT).get(0);

          // TODO: tweak these more?
          const zoomAdjustment = instance.entity.ScanIdAgeVerification.getIdTypeConfig().idRegionZoom + Math.floor(Math.random() * Config.ZOOM_ADJUSTMENT_MAX_VALUE);

          instance.debug.logToContainer('<p>Zoom: ' + zoomAdjustment + '</p>');

          const maskCanvas = Avs.Helper.VideoAreaExtractor.videoMaskToCanvas(
      ...
      public static getAgeMinimumValue() {
        ...
        const idCountry = instance.entity.ScanIdAgeVerification.idCountry;
        const idState = instance.entity.ScanIdAgeVerification.idState;
      ...
      public static repositionScanId() {
        ...
        const elementPosition = Avs.Helper.ElementPositionCalculator.getScreenPositionFromConfig({
        ...
        const innerElementPosition = Avs.Helper.ElementPositionCalculator.getScreenPositionRelativeToContainer(
          {
            width    : instance.entity.ScanIdAgeVerification.getIdTypeConfig().idSizeList.container.width,
            height   : instance.entity.ScanIdAgeVerification.getIdTypeConfig().idSizeList.container.height,
            maxWidth : Config.BOUNDARY_MAX_WIDTH,
